...
void lru_gen_del_mm(struct mm_struct *mm)
{
	...
	for_each_node(nid) {
		...
		// Comment it, trying to match MGLRU code from 6.18.x
			    ...
			    struct mm_struct **iter)
{
	...
	if (*iter)
		mmput_async(*iter);
	else if (walk->max_seq[type] <= READ_ONCE(mm_state->seq[type]))
		return false;

	spin_lock(&mm_list->lock);

	VM_BUG_ON(mm_state->seq[type] + 1 < walk->max_seq[type]);
	VM_BUG_ON(*iter && mm_state->seq[type] > walk->max_seq[type]);
	...
	if (!mm_state->nr_walkers) {
		VM_BUG_ON(mm_state->head[type] && mm_state->head[type] != &mm_list->fifo);

		mm_state->head[type] = mm_list->fifo.next;
		first = true;
	}

	while (!mm && mm_state->head[type] != &mm_list->fifo) {
		...
		if (!mm_state->tail[type] || mm_state->tail[type] == &mm->lru_gen.list) {
...
static bool iterate_mm_list_nowalk(struct lruvec *lruvec, unsigned long max_seq, int type)
{
	...
	if (max_seq > mm_state->seq[type] && !mm_state->nr_walkers) {
		VM_BUG_ON(mm_state->head[type] && mm_state->head[type] != &mm_list->fifo);
...
static int should_skip_vma(unsigned long start, unsigned long end, struct mm_walk *walk)
{
	...
	if (vma_is_anonymous(vma))
		return walk->aging_type == LRU_GEN_FILE || !priv->can_swap;
			   ...
			   struct mm_walk *walk)
{
	...
	int old_gen, new_gen = lru_gen_from_seq(priv->max_seq[walk->aging_type]);
...
restart:
	...
	for (addr = start; addr != end; i++, addr = next) {
...
#endif
		if (!priv->full_scan && !test_bloom_filter(priv->lruvec, priv->max_seq[walk->aging_type], pmd + i))
			continue;
		...
		update_bloom_filter(priv->lruvec, priv->max_seq[walk->aging_type] + 1, pmd + i);
	}
...
static bool try_to_inc_max_seq(struct lruvec *lruvec, unsigned long max_seq,
			       struct scan_control *sc, int type, bool can_swap, bool full_scan)
{
	bool success;
	...
	VM_BUG_ON(max_seq > READ_ONCE(lrugen->max_seq[type]));
	...
	if (!success) {
		if (!current_is_kswapd() && !sc->priority)
			wait_event_killable(lruvec->mm_state.wait,
					    max_seq < READ_ONCE(lrugen->max_seq[type]));

		return max_seq < READ_ONCE(lrugen->max_seq[type]);
	}
		       ...
		       unsigned long min_ttl)
{
	...
	if (nr_to_scan && need_aging && (prot != MEMCG_PROT_LOW || sc->memcg_low_reclaim))
		try_to_inc_max_seq(lruvec, max_seq, sc, type, swappiness, false);
...
void lru_gen_look_around(struct page_vma_mapped_walk *pvmw)
{
	...
	struct lruvec *lruvec = mem_cgroup_lruvec(pgdat, memcg);
	int ptype = page_is_file_cache(page);
	DEFINE_MAX_SEQ(lruvec);
	int old_gen, new_gen = lru_gen_from_seq(max_seq[ptype]);
	...
	if (suitable_to_scan(i, young))
		update_bloom_filter(lruvec, max_seq[ptype], pvmw->pmd);
...
static int get_type_to_scan(struct lruvec *lruvec, int swappiness, int *tier_idx)
{
	...
	/*
	 * Compare the first tier of anon with that of file to determine which type to scan. 
	 * This uses refault rates as feedback to adapt to changing workloads.
	 * Also need to compare other tiers of the selected type
