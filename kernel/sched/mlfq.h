 ...
 *
 * The block sits between the clamps and the code that reads them because it is
 * the last of the constants it is about: the classifier compares against the
 * effective edges and not against the base thresholds, so most of what follows
 * is a statement about the ranges the controller moves within, and the clamps
 * are the last thing that has to be defined before any of it can be said.
 */
...
/*
 * An effective edge is its base threshold scaled and then clamped, so a clamp
 * that did not contain its own base would mean the zero shift that a disabled
 * controller leaves in force did not give the threshold back, and the base
 * pair would sit outside the ranges the classifier is bounded to.
 */
static_assert(MLFQ_T_L_FLOOR_NS <= MLFQ_THRESH_LOW_NS &&
	      MLFQ_THRESH_LOW_NS <= MLFQ_T_L_CEIL_NS,
	      "the interactive edge is outside its own clamp, so a zero shift "
	      "does not return the base threshold");

static_assert(MLFQ_T_H_FLOOR_NS <= MLFQ_THRESH_HIGH_NS &&
	      MLFQ_THRESH_HIGH_NS <= MLFQ_T_H_CEIL_NS,
	      "the CPU-bound edge is outside its own clamp, so a zero shift "
	      "does not return the base threshold");

/*
 * The clamped ranges are disjoint. That is what keeps the band ordered at every
 * admissible shift -- the interactive edge's ceiling is under the CPU-bound
 * edge's floor, and the base pair is inside those, so the base band is ordered
 * with them -- and it is also what makes a controller step landing between the
 * two loads in mlfq_read_bands() harmless, since a low edge from one step read
 * against a high edge from the next is still an ordered band at least 700us
 * wide, which is what the comment on the clamps claims.
 */
static_assert(MLFQ_T_L_CEIL_NS < MLFQ_T_H_FLOOR_NS,
	      "the clamps let the adapted edges cross, so a classification can "
	      "see an empty band and the hysteresis gates cross with it");

/*
 * The widest the CPU-bound edge can be adapted to is still under the ceiling
 * the gauge climbs towards, so a task that never sleeps reaches it at every
 * shift and the demotion to Q3 stays reachable however the controller moves.
 */
static_assert(MLFQ_T_H_CEIL_NS < MLFQ_BUDGET_MAX_NS,
	      "the CPU-bound edge can be adapted past the gauge ceiling, so a "
	      "task the controller widened the band for is never demoted");

/*
 * mlfq_ema_climb() multiplies the gap, the running time and the coefficient
 * together before dividing, so all three have to stay far enough below 2^64
 * that the product cannot wrap. The clamp that keeps the first two apart is
 * only as good as the ceiling they are clamped to.
 */
static_assert(MLFQ_BUDGET_MAX_NS * MLFQ_BUDGET_MAX_NS * MLFQ_EMA_ALPHA_FP
	      < (1ULL << 63),
	      "the gauge climb can overflow 64 bits, which wraps the gauge "
	      "instead of saturating it");

/*
 * The 177 in mlfq_ema_decay() is MLFQ_FP_ONE * ln(2) truncated, which is what
 * makes the second-order expansion there an expansion of 2^-x. It is a literal
 * because it is a constant of the arithmetic rather than of the policy, and it
 * is silently wrong the moment the shift it is written in changes.
 */
static_assert((MLFQ_FP_ONE * 693) / 1000 == 177,
	      "the ln(2) constant in mlfq_ema_decay() is in MLFQ_FP_ONE fixed "
	      "point, so it has to be recomputed when MLFQ_FP_SHIFT changes");
