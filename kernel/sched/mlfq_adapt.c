 ...
 *
 * All of it is per-CPU and written from the CPU that owns it, under that
 * runqueue's lock, so none of it costs a shared cacheline on any scheduling
 * path. Readers sum across CPUs and are not serialised against the writers;
