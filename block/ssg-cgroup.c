...
static inline struct blk_mq_tags *ssg_blkcg_sched_tags(struct request_queue *q)
{
	if (unlikely(!q->queue_hw_ctx || !q->queue_hw_ctx[0]))
		return NULL;

	return q->queue_hw_ctx[0]->sched_tags;
}
...
static void ssg_blkcg_pd_init(struct blkg_policy_data *pd)
{
	...
	ssg_blkcg_set_shallow_depth(ssg_blkcg, ssg_blkg,
			ssg_blkcg_sched_tags(pd->blkg->q));
}
