...
/*
 * This kernel has no blkcg_css(). bio_blkcg(NULL) is its exact equivalent:
 * kthread_blkcg() when set, otherwise the current task's io cgroup css.
 * Must be called with rcu_read_lock() held.
 */
static inline struct blkcg *ssg_current_blkcg(void)
{
	return bio_blkcg(NULL);
}
 ...
 * ssg-wb.c
 *
 * Not ported: the upstream write booster is a client of Samsung's out-of-tree
 * "blk-sec" layer (blk_sec_wb_ctrl() / blk_sec_wb_is_supported() /
 * blk_sec_common_wq), which does not exist in this tree, and this tree already
 * drives UFS WriteBooster from ufshcd devfreq clock scaling. The hook seam is
 * kept so the file can be dropped in later without touching ssg-iosched.c.
 */
